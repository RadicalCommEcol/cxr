...
Another feature of `cxr` is the projection of population abundances in time given a population dynamics model and a set of parameters. This is particularly useful when we want to know how our predictions match observed changes in the number of individuals across time or space. This feature is available for predictive models with the same set of parameters as the ones provided (i.e. **lambda**, **alpha**, **lambda.cov**, and **alpha.cov**). In the package we include a predictive version of the five Beverton-Holt fecundity models, but users are encouraged to code their own predictive models.

In this document, we exemplify the use of our projection function (`predict????`). We project the abundances of a set of species from a simulated dataset, in order to show the usage of the function `GenerateTestData`, also included in the package.
...
# lambda values
focal.lambda <- runif(num.sp,50,1000) #we draw between 50 and 1000 seeds per individual.
# alpha values
...
[IB] Can we add an example of calculating AIC elswhere? 