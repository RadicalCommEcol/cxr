...
The `competition` dataset includes observation from *20* different plant species (see vignette `Data_formats` for details on the included datasets). We will subset the information to consider only two of them, *Chamaemelum fuscatum* and *Leontodon maroccanus*.
...
In order to estimate the different parameters for both species, we need to store separately the information on their observations as focal species.
...
and also the estimation of fitness assocaited with each observation. In this particular case this is the seed production of focal individuals, for which the number of neighbours (`comp.matrix`) were also taken from field observations. Note that we pass the log of the seed production (fitness metric) for facilitating the optimization procedure.
...
sp1.par <- cxr_optimize(fitness.model = BH_3,
                        ...
                        lower.lambda = 0, #negative numbers of seed production are not biologically meaningful, hence we fix lambda to a minimum of 0.
                        upper.lambda = 1e3,
                        init.sigma = sd(sp1.log.fitness),
                        lower.sigma = 1e-5,
                        upper.sigma = 1,
                        init.alpha = c(0,0), #initial competition coeficients set to no competition. Postive numbers imply competition, and negative numbers facilitation.
                        lower.alpha = c(-1e3,-1e3),
...
and obtain the niche overlap and average fitness ratio between the two species [IB define this metrics, maybe?]
