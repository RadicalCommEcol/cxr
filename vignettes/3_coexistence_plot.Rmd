...
library(cxr)
data(param_estimates) #it's not when loading it twice that R crashes, but when using outcome in line.
data(species_rates)
```
...
coex.plot <- ggplot2::ggplot() + 
  ...
  NULL
coex.plot #Can we add a legend? Ideally indicate wich one is coexistence and which one is exclusion.
```
