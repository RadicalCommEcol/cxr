...
```{r}
my.sp <- c("CHFU", "LEMA")
two.sp.data <- subset(competition, focal.sp %in% my.sp & competitor %in% my.sp)
```
...
sp1.comp.matrix <- comp.matrix[which(two.sp.data$focal == my.sp[1]),]
sp2.comp.matrix <- comp.matrix[which(two.sp.data$focal == my.sp[2]),]
...
```{r}
interaction.matrix <- matrix(c(sp1.par$alpha, sp2.par$alpha), byrow = TRUE)
```
...
overlap <- NicheOverlap()
fitness.ratio <- AverageFitnessRatio()
