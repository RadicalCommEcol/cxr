...
```{r}
?competition
head(competition) #fruit and seed are then duplicated... suboptimal. Add individual.
```
...
In the package there are, broadly speaking, two types of functions. 1) `cxr_optimize`, which fits the species interaction models to empirical data and 2) `ER_optimize`, which fits overall response and effect parameters per focal species to empirical data. Both `cxr_optimize` and `ER_optimize` accept field data and numerically estimate parameters of a model provided by the user. On the other hand, other functions (`AvgFitnessRatio`, `NicheOverlap`, or `CompetitiveAbility`) take these estimates and return coexistence metrics for a single species or pairs of them.
...
where $r_i$ is the species’ competitive response and $e_i$ is the species’ competitive effect; our models only differ on whether is taken as a fixed parameter (function `EffectResponse.R`) or is optimized as well in the routine (function `EffectResponse_lambda.R`).[move up?]
