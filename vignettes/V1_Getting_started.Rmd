...
```{r}
# create two stochastic covariates
# with values for three sp
example_cov <- list()
for(i.sp in 1:length(obs_3sp)){
  cov1 <- runif(nrow(obs_3sp[[i.sp]]),0,1)
  cov2 <- runif(nrow(obs_3sp[[i.sp]]),10,100)
  example_cov[[i.sp]] <- matrix(c(cov1,cov2),
                                nrow = nrow(obs_3sp[[i.sp]]),
                                ncol = 2)
  colnames(example_cov[[i.sp]]) <- c("cov1","cov2")
}

names(example_cov) <- names(obs_3sp)
...
fit_multi_cov <- cxr_pm_multifit(data = obs_3sp,
                           ...
                           initial_values = list(lambda = 1,
                                                 alpha_intra = 0.1,
                                                 alpha_inter = 0.1,
                                                 lambda_cov = c(0.1,0.01),
                                                 alpha_cov = c(0.1,0.01)),
                           # note how lambda_cov and alpha_cov
                           # have different boundaries for each covariate effect
                           lower_bounds = list(lambda = 0,
                                               alpha_intra = 0,
                                               alpha_inter = -1,
                                               lambda_cov = c(0,-1),
...
# covariate terms
fit_multi_cov$lambda_cov
fit_multi_cov$alpha_cov
```
