...
If we set the flag, the nested list will be written to disk. The list structure is very convenient for handling these results in R, as it maintains the nested design of the parameterization (per species, model, and optimization method). However, it may also be necessary to have the results as dataframes and csv files, e.g. for plotting. Here we build four dataframes for the different sets of parameters: lambda, alpha, lambda.cov, and alpha.cov and store all results. This assumes that covariates are named, so the vector from the list are also named.
