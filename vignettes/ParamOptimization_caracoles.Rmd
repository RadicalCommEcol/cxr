...
We also need to specify the initial values for the first model (model 3, in this example). If you have independent estimates of any parameter, comment out the appropriate lines. Note that some of the optimization methods (e.g. \code{optim-LFGS-B}) fail with lower bounds equal to zero. Hence the small, but positive, values.
...
With this setting, we are now ready to optimize the selected parameters based on our dataset. This is done species by species. As the main function, \code{cxr_optimize}, accepts the input for a single species, we code a loop for going through all species. Inside this loop, we include the following steps:
