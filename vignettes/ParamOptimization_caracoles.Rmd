...
- \code{salinity}: soil salinity for each subplot. This is our abiotic covariate, but any other types of covariates can be included. 
...
This part consists in two different points. First we build a data structure that will hold the results for all species, and second we prepare the example database for the analysis.
...
Importantly, the package allow computing estimations of standard errors. This error estimation is done via bootstrapping by two reasons. One is that obtaining estimation of error by Hessian Matrix is unstable due to several computational issues, but sencondly and more importantly, error distributions of your variable might not follow a normal distribution. Please remember that the estimation of errors can be, depending on the model and optimization method, computationally expensive. We recommend 999 runs, yet we provide here a minimal example of it (see bootstrap.samples).
...
With this procedure, we are now ready to optimize the data set. This is done species by species. As the main function, \code{cxr_optimize}, accepts the input for a single species, we code a loop for going through all species. Inside this loop, we include the following steps:
- 1) subset the competition data for the focal species 
- 2) generate the actual initial values, considering all situations. Remember that, depending on the model, some parameters will have different length. For example, $\alpha$ is a single value in model 2 and a matrix in model 3 onwards. This matrix will not be squared if the number of focal species for which fecundity is predicted is lower than the number of competitors. Thus, for species *i*, $alpha_{ij}$ is a vector of length equal to the number of competitor species.
