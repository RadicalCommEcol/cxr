...
Again, a simple plot showing the variations in competitive effects and responses among species. For that, we have to format the data into a dataframe

```{r}
er.data <- data.frame(species = sp.names, competitive.effect = ER.list[[1]]$effect, competitive.response = ER.list[[1]]$response)

er.data.long <- gather(er.data,key = "var",value = "value",-species)

er.plot <- ggplot(er.data.long,aes(y = species, x = value, group = var))+
  geom_point()+
