...
Since we have independent estimates for $\lambda$, we are treating it as a fixed parameter that we do not want to fit. This can be passed on to the \code{ER_optimize} function by choosing the appropriate effect/response model. Note that, if we are not interested in fitting $\lambda$, we can dispense with specifying lower/upper bounds for it.
...
It is possible to estimate, alongside competitive effects and responses, the effect of covariates over these terms. For simplicity, we will assume no effect of covariates in this example, but their addition is similar to the one exemplified in the vignette \code{Multi-species parameter optimization}. We can assume that seed production does not change in a single year, so group observations in year x site records, where a site is a subplot within a specific plot (e.g. subplot A1 of plot 1)
