...
This document explains how to optimize several models that predict species fecundity. This means that models with increasing number is a more complex version of the previous model. This procedure is intended to avoid overfitting problems and promote convergence of the more complex models. Here, we present how to use data obtained from experiments or field observations to predict species fecundity as a function of the intrinsic ability of species to produce offspring ($\lambda$), and interaction coefficients with other species ($\alpha$). The model in its most simpler version takes the following form:

$$F = \frac{\lambda}{1+\alpha}$$

This form is basically a Beverton-Holt model. Following modifications of this model will allow us obtaining the interaction matrices for pairs of species, and the effect of covariables (including biotic and abiotic factors) on both $\lambda$ and $\alpha$ coefficients (see vignette `Data and model formats` for details on the five models implemented). All together this information is key for obtaining information of different drivers determining the outcome of species interaction at local scales, which are widely used in "modern" coexistence studies. Most of this vignette presents data preparation from how is obtained in the field to the appropriate format for the analysis. 

**Setup**

We start loading the package and the associated dataset (see vignette `Data and model formats` for details on the data). We will use the following data:
...
library(cxr)
data(competition, package = "cxr") #R crashes if I load data twice :() Error in class(obj) <- "rs.scalar" : cannot set attribute on a symbol
...
First we need to format the example dataset for the analysis and add the initial values needed as inputs for model parametrization.
...
```{r message=F, warning=F}
models <- 3:5 # Ideally, you should start by model 1, and build from there, here we start at model 3 for demonstration purposes
```

This sequential procedure means tha the output from model 3 will be taken as initial estimates for model 4, and the output from model 4 will be the initial estimates for model 5. Note, as well, that models 4 and 5 differ from model 3 in that they include the effect of covariates on species seed production in the absence of competition and interactions. Therefore, you should only include these models if you have at least one covariate in your data (see *Lanuza, Bartomeus, Godoy. 2018. Ecology Letters 21:865-874*).
...
param.list <- list(m3 = c("lambda","alpha"),
...
```{r message=F, warning=F}
#param.list <- list(m3 = c(alpha"),
 #                  m4 = c("alpha","lambda.cov","alpha.cov"),
 #                  m5 = c("alpha","lambda.cov","alpha.cov")) 
# init.lambda <- rep(10,length(focal.sp)) # As an example, here we force the value of lambda to take the value of 10 for all species.
```
...
Here we specify which optimization methods to use. You can include more than one (but see below). In this case we only use optim_NM because it is one of the default methods.
...
We also need to specify the initial values for the first model (model 3, in this example). If you have independent estimates of any parameter, comment out the appropriate lines. Note that some of the optimization methods (e.g. `optim-LFGS-B`) fail for parameter combinations in which the function is undefined. Hence the small, but positive, values.

```{r message=F, warning=F}
# values for initial estimates of parameters. 
# Overwrite if necessary
# init.lambda is calculated after log.fitness for each focal species
lower.lambda <- 1 # asumption that at minium it will produce 1 seed. We avoid zero seeds scenario, because it precludes the use of log().
upper.lambda <- 1e3
# sigma
lower.sigma <- 1e-5
upper.sigma <- 1
# alpha
init.alpha <- 0 # DAVID: As in the first example
lower.alpha <- 1e-10
upper.alpha <- 1e5
# lambda.cov
init.lambda.cov <- 1
...
for(i.sp in 1:length(focal.sp)){
  ...
  for(i.model in 1:length(models)){
    ...
    for(i.method in 1:length(optim.methods)){
      param_estimates[[i.sp]][[i.model]][[i.method]] <- list(lambda = NA, #turn to NA if possible 
...
With this setting, we are now ready to optimize the selected parameters based on our dataset. This is done species by species. As the main function, `cxr_optimize`, accepts the input for a single species, we code a loop for going through all species. Inside this loop, we include the following steps:
...
- 3) the actual call to `cxr_optimize`
...
for(i.sp in 1:length(focal.sp)){
  ...
  focal.sp.data <- subset(focal.sp.data, seed > 0) #OSCAR, really? Oscar dice que adelante con el +0.01
...
If we set the flag, the nested list will be written to disk as a .RData file. The list structure is very convenient for handling these results in R, as it maintains the nested design of the parameterization (per species, model, and optimization method). However, it may also be necessary to have the results as dataframes and csv files, e.g. for plotting. Here we build four dataframes, in long format, for the different sets of parameters: lambda, alpha, lambda.cov, and alpha.cov. This code assumes that covariates are named, so that vectors returned from the list are also named.
...
head(lambda.values)
head(alpha.values)
head(lambda.cov.values) #NA's (maybe only for model 3?)
head(alpha.cov.values) #0's (maybe only for model 3?)
