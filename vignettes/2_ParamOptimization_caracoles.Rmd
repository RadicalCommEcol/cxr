...
Remember that if we include more than one model, the optimization procedures proceed sequentially.

```{r message=F, warning=F}
models <- 1:5 
```

Again, this sequential procedure means tha the output from model 1 will be taken as initial estimates for model 2, the output from model 2 will be the initial estimates for model 3, and so on. Note, as well, that models 4 and 5 differ from model 3 in that they include the effect of covariates on species seed production in the absence of competition and interactions. Therefore, you should only include these models if you have at least one covariate in your data (see *Lanuza, Bartomeus, Godoy. 2018. Ecology Letters 21:865-874*).
...
In case you had independent estimates of parameters, e.g. $\lambda$, you would have something along these lines:
...
# init.lambda <- rep(10,length(focal.sp)) 
# As an example, set the value of lambda to 10 for all species.
```
...
for(i.sp in 1:length(focal.sp)){
  ...
  # our fitness metric is simply number of seeds
...
If we set the `write.results` flag, the nested list will be written to disk as a .RData file. The list structure is very convenient for handling these results in R, as it maintains the nested design of the parameterization (per species, model, and optimization method). However, it may also be necessary to have the results as dataframes and csv files, e.g. for plotting. Here we build four dataframes, in long format, for the different sets of parameters: lambda, alpha, lambda.cov, and alpha.cov. This code assumes that covariates are named, so that vectors returned from the list are also named.
...
if(write.results){
  ...
  lambda.values$lambda <- NA
  lambda.values$lambda.lower <- NA
  lambda.values$lambda.upper <- NA
  
  # alpha
  alpha.values <- expand.grid(focal.sp,competitors,my.models,optim.methods)
  names(alpha.values) <- c("focal","competitor","model","method")
  alpha.values$alpha <- NA
  alpha.values$alpha.lower <- NA
  alpha.values$alpha.upper <- NA
  ...
  lambda.cov.values$lambda.cov <- NA
  lambda.cov.values$lambda.cov.lower <- NA
  lambda.cov.values$lambda.cov.upper <- NA
  ...
  alpha.cov.values$alpha.cov <- NA
  alpha.cov.values$alpha.cov.lower <- NA
  alpha.cov.values$alpha.cov.upper <- NA
...
```{r}
  head(alpha.values[alpha.values$model == "BH_2",]) # first model to fit alpha
```

And for $\Theta$, or `lambda.cov` in the models, representing the effect of covariates over $\lambda$

```{r}
  head(lambda.cov.values[lambda.cov.values$model == "BH_4",]) # first model to fit lambda.cov
```

Lastly, $\phi$ or `alpha.cov` represents the effect of covariates over $\alpha$

```{r}
  head(alpha.cov.values[alpha.cov.values$model == "BH_4",]) # first model to fit alpha.cov
```
