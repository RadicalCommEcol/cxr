...
Since we have independent estimates for $\lambda$, we are taking it as a fixed parameter that we do not want to fit. This can be passed on to the `ER_optimize` function by choosing the appropriate effect/response model. Note that, if we are not interested in fitting $\lambda$, we can dispense with specifying lower/upper bounds for it.

```{r message=F, warning=F}
optimize.lambda <- FALSE
# As the model is different for the two cases, we selct the right model in each case.
if(optimize.lambda){
  ...
  lower.lambda <- NA
  upper.lambda <- NA
  #yo daria el valor de lambra init aqui.
} 
```
...
It is possible to estimate, alongside competitive effects and responses, the effect of covariates over these terms. For simplicity, we will assume no effect of covariates in this example, but their addition is similar to the workflow exemplified in the vignette `Multi-species parameter optimization`. The dataset included with the package includes observations from 2016, but here we provide general code for any number of years (or sampling campaigns). We group observations in year x site records, where a site is a subplot within a specific plot (e.g. subplot A1 of plot 1).

```{r message=F, warning=F}
sp.data <- competition %>% dplyr::group_by(year,plot,subplot,focal,competitor) %>% dplyr::summarise(seed = sum(seed),number = sum(number)) # As there is only one value per group, the sum returns that seed value.
...
sigma <- mean(lambda.values$sigma) #entra solo como valor inicial, no fijo.
```

Double-check that we only include species that have proper estimates

```{r message=F, warning=F}
#check first and remove first. Or remove from  data.
...
```{r message=F, warning=F}
r.values <- rep(NA,nrow(lambda.values))
e.values <- rep(NA,nrow(lambda.values))
...
We can now initialize an empty results data structure, a list in which each component will consist of the estimates from an optimization method.
...
In the main loop, we 1) call the `ER_optimize` function with each of the selected optimization methods, 2) store the resulting values in the list we just created. Note that, in the function call, we omit the arguments related to the inclusion of covariates, because these have a default `NULL` value. These arguments (e.g. `lambda.cov`, `r.cov`, `e.cov`, and their associated errors) are nevertheless returned as NA from the function, and for completeness we also retrieve them.
