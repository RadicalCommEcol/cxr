...
In this document, we draw the coexistence region of our plant community and locate every pair of species in this region accroding to two orthogonal axes. Average fitness differences (y-axis) and niche differences (x-axis). For that, we use the parameters previously calculated for each species (see vignette \code{Multi-species parameter optimization}), and the germination and seed soil bank survival rates, which are both stored with the package. Please note that germination and seed survival rates are a particular feature of the annual plant model (see Godoy and Levine 2014 Ecology 95(3) 726-736 for details), but other models might be adapted to other species particularities. 
...
For drawing the plot, we adapt, simplified, the great example from Ke and Letten (2018) Nature Ecology and Evolution 2(11) 1691, in which they show the regions leading to coexistence, exclusion, and priority effects between species pairs.
