...
Here we specify which optimization methods to use. You can include more than one (but see below). In this example, we use the Nelder-Mead algorithm because it is the default method in the `optim` and `optimx` packages.

```{r message=F, warning=F}
# optimization methods to use
optim.methods <- c("Nelder-Mead" 
                   # "optim_L-BFGS-B"
                  # "nloptr_CRS2_LM"
                   # "nloptr_ISRES"
...
We also need to specify the initial values for the first model (model 1, in this example). If you have independent estimates of any parameter, comment out the appropriate lines. The `Nelder-Mead` method used in this example does not account for upper and lower parameter values, but we include them here for completeness.
