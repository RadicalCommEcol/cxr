...
Both `pm_optim` and `er_optim` are built on top of numerical optimization procedures The methods available come, mainly, from the `optimx` package, which is a general, more robust replacement for the base `optim`. Other methods, stemming from the analyses in *Mullen, K.M. 2014. Journal of Statistical Software 60:1–45*, were included specifically. The methods available are:

```{r}
cxr::all.methods
```

Some of these methods need additional packages to be installed; `pm_optim` will throw an informative error in these cases. Furthermore, methods differ on whether they optimize parameters with or without so-called box-constraints, i.e. lower and upper parameter bounds. Specifying these bounds for a boundary-free method will discard them, whereas if bounds are not specified for boxed methods, the default boundaries of `pm_optim` and `er_optim` will be used.
