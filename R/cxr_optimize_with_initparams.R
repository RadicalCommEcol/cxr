...
cxr_optimize2 <- function(fitness.model,
                          optim.method,
                          param.list, #lambda,sigma,alpha.single,alpha.matrix,lambda.cov,alpha.cov.single,alpha.cov.matrix
                          log.fitness,
                          init.lambda = NULL,
                          lower.lambda = 1,
                          upper.lambda = 1e5,
  ...
  init.par <- InitParams_2(init.lambda = my.init.lambda,
  ...
  # optim.result <- list(par = rep(NA,length(init.par)), value = NA)
