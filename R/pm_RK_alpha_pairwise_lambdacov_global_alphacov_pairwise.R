#' Ricker model with pairwise alphas, covariate effects on lambda, 
...
#' @export
pm_RK_alpha_pairwise_lambdacov_global_alphacov_pairwise <- function(par,
                                                                    fitness,
  ...
  for(z in 0:(ncol(all_neigh_matrix)-1)){
    ...
    alpha_cov_term[[z+1]] <- cov_term_x_sum
  }
  ...
  for(z in 1:ncol(all_neigh_matrix)){
    interactions_term <- interactions_term - (alpha[z] + alpha_cov_term[[z]]) * all_neigh_matrix[,z] 
  }
