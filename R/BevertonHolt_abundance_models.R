...
#' @param cov.values Not used in BH_abundances_3
...
BH_abundance_3 <- function(sp.par,init.abund,cov.values,alpha.matrix,lambda.cov.matrix,alpha.cov.matrix){
  ...
  # the above formulation gives number of seeds. Multiply it by the germination rate to get number of individuals
  expected.abund*sp.par$germ.rate
}
...
#' @param alpha.matrix competition matrix
#' @param lambda.cov.matrix matrix of dimension number of species x number of covariates
...
#' Each component of the list is a matrix of dimension number of species x number of species,
