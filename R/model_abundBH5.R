...
#' Each component of the list is a matrix of num.sp x num.sp,
...
#' @export
model_abundBH5 <- function(sp.par,init.abund,cov.values,alpha.matrix,lambda.cov.matrix,alpha.cov.matrix,return.seeds = TRUE){
  ...
  for(i.sp in 1:nrow(sp.par)){
    ...
    if(return.seeds){
      ...
      for(j.sp in 1:nrow(sp.par)){
        ...
        den <- den + alpha.term*(init.abund[j.sp]*sp.par$germ.rate[j.sp])
      }# for j.sp
    }else{
