...
#' @export
#'
er_optim <- function(lambda.vector,
                        e.vector,
                        r.vector,
                        lambda.cov = NULL,
                        e.cov = NULL,
  ...
  if(!is.null(covariates)){
    if(is.null(lambda.cov) | is.null(e.cov) | is.null(r.cov) | is.null(lower.lambda.cov) | is.null(upper.lambda.cov) | is.null(lower.e.cov) | is.null(upper.e.cov)
      ...
      stop("er_optim ERROR: Covariates are given but initial values/bounds for lambda.cov, r.cov, or e.cov are missing")
    }
  }
  ...
  if (optim.method == "GenSA" & !requireNamespace("GenSA", quietly = TRUE)) {
    stop("er_optim ERROR: Package \"GenSA\" needed for the method selected to work.",
         call. = FALSE)
  }
  if (optim.method == "hydroPSO" & !requireNamespace("hydroPSO", quietly = TRUE)) {
    stop("er_optim ERROR: Package \"hydroPSO\" needed for the method selected to work.",
         call. = FALSE)
  }
  if (optim.method == "DEoptimR" & !requireNamespace("DEoptimR", quietly = TRUE)) {
    stop("er_optim ERROR: Package \"DEoptimR\" needed for the method selected to work.",
         call. = FALSE)
  }
  # fill up matrices
  ...
  if(generate.errors){
    errors <- cxr_er_bootstrap(effect.response.model = effect.response.model,
