...
# function to optimize r and e parameters (competition responses/effects)
# see Godoy et al. 2014.
# here, all sp have to be parameterized in the same function, because e
# is not pair-specific.
# Therefore, the vector of parameters includes all r, all e, and the sigma term

# species are sorted alphabetically

effect_response_model <- function(par, sp.data, lambda.vector){
  ...
  r.vector <- par[1:num.focal]
  # r.vector <- par[(num.focal+1):(num.focal+num.focal)]
  e.vector <- par[(num.focal+1):(length(par)-1)]
