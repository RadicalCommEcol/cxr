...
#' @export
LV_project_alpha_pairwise_lambdacov_none_alphacov_none <- function(lambda,
                                                                   alpha_intra,
                                                                   alpha_inter,
                                                                   lambda_cov,
                                                                   alpha_cov,
  ...
  term <- 0
  for(i.sp in 1:numsp){
    term <- term - alpha[i.sp]*abundance[i.sp]
  }# for each sp
  
  expected_abund <- (lambda - term)* abundance[names(lambda)]
  expected_abund
